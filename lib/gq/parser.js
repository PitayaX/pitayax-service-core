...
class Parser
{
    ...
    {
        let p = new Parser();
        return p.parseFileSync(scriptPath);
    }
    ...
    parseFile(scriptPath, callback) {
        ...
        return (
            aq.readFile(scriptPath, {encoding: 'utf-8'})
                ...
                .then(script => (callback) ? callback(null, script) : aq.Q(script))
                .catch(err => (callback) ? callback(err, null) : aq.Q(null, err))
        );
    }
    ...
    parseContent(content) {
        ...
        let sc = (() => {
                try{
                    ...
                    throw new Error('Invaild script content');
                }
        })();
