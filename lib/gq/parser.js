...
class Parser
{
    ...
    parseContent(content) {
        ...
        if (sc) {
            ...
            if (sc.arguments) {
                $sc.arguments = Object.keys(sc.arguments)
            ...
            if (sc.beforeEach) $sc.beforeEach = sc.beforeEach;
            if (sc.afterEach) $sc.afterEach = sc.afterEach;
        }
    ...
    parsePart(owner, parent, part) {
        ...
        if (part.headers) {
            Object.keys(part.headers)
                .forEach(header => $part.headers[header] = part.headers[header]);
        }
        ...
        if (part.parts) {
            if (Array.isArray(part.parts)) {
                ...
                part.parts.forEach(part => that.parsePart(owner, $part.parts, part));
            }
