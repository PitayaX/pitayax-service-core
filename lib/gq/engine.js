...
class Engine
{
  ...
  fillRelation(ctx, data, relation)
  {
    ...
    if (keys.length == 1)
    {
      let
        mapKeys = data
                    ...
                    .filter( item => (item) ? true : false)   //remove undefined items
        mapKeys = mapKeys.filter( (item, pos) => mapKeys.indexOf(item) === pos )  //remove duplication items

      //set headers
      headers['method'] = 'retrieve'
      headers['target'] = relation.Target
      headers['options'] = {}
      headers['options']['fields'] = Array.from(relation.As.Fields)
    ...
    return that
      //.execAction(ctx, action, headers, {"query": {"userToken": {"$in": ['Bruce']}}})
      ...
      .then( rdata => {
        for( let i = 0; i < data.length; i++ )
        {
          ...
          for( let j = 0; j < keys.length; j++)
          {
            ...
            mapData = mapData.filter( ritem => ritem[relation.Joins.get(keys[j])] === row[keys[j]])
          }

          const mapRow = (mapData !== undefined && mapData.length > 0) ? mapData[0] : {}
  ...
  getContext(parent)
  {
    ...
    if (!ctx.global)
    {
      ...
      ctx.global.repackage = (data) => {
        ...
        if (Array.isArray(data))
          return data.map( row => (typeof row.toObject === 'function') ? row.toObject() : row )
